...
The project has a working offline SQLite pipeline, Football-Data CSV import, historical replay, Elo and baseline prediction modes, replay comparison reports, model configuration metadata, comparison ranking annotations, deterministic comparison analysis output, and a deployed React dashboard.
...
The next full roadmap for real-time Misli paper recommendations is documented in `docs/superpowers/plans/2026-05-22-live-misli-recommendations.md`. Task 50 is complete: `run-scheduled-paper-worker` is a one-shot, scheduler-safe command that requires `LIVE_COLLECTION_ENABLED=true`, records `scheduled_paper_worker` live runs, skips overlapping worker executions, and delegates to the scoped live paper cycle. Task 53 is complete: Misli public parsing now normalizes comma odds, rejects empty identity rows, records parser-drift/low-confidence live-run errors, and surfaces those states in provider-health analysis. Task 54 is complete: read-only odds movement summaries are available through `GET /api/live/odds-movement` and the dashboard data layer. Task 55 is complete: deterministic paper recommendations persist to `paper_recommendations` and are exposed through `generate-recommendations` plus `GET /api/live/recommendations`. Task 56 is complete: ranked paper-only combinations persist to `paper_combinations` and are exposed through `generate-combinations` plus `GET /api/live/combinations`. Task 57 is complete: deterministic AI recommendation review persists `recommendation_review` analyses and is exposed through `analyze-recommendations` plus `GET /api/ai/recommendation-review/latest`. Task 58 is complete: the React dashboard now shows filterable live recommendations, combinations, odds movement, risk flags, and AI approval state. Task 59 is complete: historical recommendation backtests export singles/combination metrics, dashboard-compatible companion reports, and `recommendation_backtest_summary` AI analyses. Task 51 is complete: Railway deployment/runbook steps and `production-smoke` checks are available. Task 60 is complete: `/api/live/worker-status` and production smoke checks now verify worker freshness and recommendation endpoint health. Task 61 is complete: `/api/operations/guardrails`, `operational-status`, and the dashboard guardrails panel distinguish warning and critical states. Task 62 is complete: `docs/deployment/production-readiness-review.md` records the conditional paper-only staging readiness decision. Task 63 is complete: the Railway API is deployed from `railway.json` plus `Dockerfile`, uses Railway Postgres, has redacted database URL logging, and deployed API `production-smoke` passes after a one-off scheduled paper worker run. Task 64 is complete: the dashboard is deployed at `https://dashboard-production-0a69.up.railway.app`, wired to the Railway API, and deployed dashboard smoke passes. Task 65 is complete: the `worker` Railway service runs `Dockerfile.worker` on cron schedule `*/30 * * * *`, and a cron-triggered run completed at `2026-05-24T14:01:20Z`. Task 66 is complete: the dashboard product focus is now a simplified "Daily Betting Card" first, with historical model analytics moved into collapsible diagnostics. Task 67 is complete: the worker can consume a fresh HTTPS snapshot JSON through `WORKER_SNAPSHOT_URL` and now generates recommendations, combinations, and AI review after each successful cycle. Task 68 is deployed: the API stores/serves latest provider snapshots, the Misli Playwright producer can post fresh public snapshot JSON, and Railway has a scheduled `snapshot-producer` service. Task 69 is complete locally: bare Misli `HH:MM` rows resolve against snapshot `scraped_at`. Task 76 is complete: combinations are experimental and excluded from primary daily decisions. Task 77 is complete: recommendation backtests produce conservative threshold advice that appears in AI summaries, daily journals, and the dashboard. Next live-provider work is deploying Task 69 and proving a completed fresh worker run.
