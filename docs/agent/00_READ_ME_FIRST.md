...
The next full roadmap for real-time Misli paper recommendations is documented in `docs/superpowers/plans/2026-05-22-live-misli-recommendations.md`. Tasks 50 through 81 have built the scheduler-safe worker, public Misli snapshot ingestion, odds movement, recommendations, combinations, AI review, backtests, guardrails, production behavior monitor, threshold review, and daily paper journal. Task 82 proved the deployed loop on 2026-06-07 from snapshot producer to API to scheduled worker to recommendation quality, threshold review, daily journal, behavior monitor, guardrails, and dashboard. Task 83 added the controlled outcome-driven threshold policy layer. Task 84 selected Football-Data CSV as the first approved external football context source and added external-context provenance/backtest visibility.
