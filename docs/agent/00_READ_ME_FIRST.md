...
The next full roadmap for real-time Misli paper recommendations is documented in `docs/superpowers/plans/2026-05-22-live-misli-recommendations.md`. Task 50 is complete: `run-scheduled-paper-worker` is a one-shot, scheduler-safe command that requires `LIVE_COLLECTION_ENABLED=true`, records `scheduled_paper_worker` live runs, skips overlapping worker executions, and delegates to the scoped live paper cycle. Task 53 is the immediate next implementation task, followed by Tasks 54 through 62 for odds movement, recommendations, combinations, AI review, dashboard recommendation views, backtesting, Railway monitoring, guardrails, and final readiness review.
